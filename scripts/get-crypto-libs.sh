...
#
# The feature is ON by default. To opt out (i.e. you have the crypto libs
# installed system-wide and want cabal to use those), set
#
#   PLINTH_USE_SYSTEM_CRYPTO_LIBS=1
#
# in which case this script is a no-op.
#
...
Options:
  ...
  --prefix DIR          Install into DIR (e.g. /usr/local or ~/.local) instead
                        ...
                        (cabal.project.local, env.sh) are NOT touched: build
                        with PLINTH_USE_SYSTEM_CRYPTO_LIBS=1 and make sure
                        DIR/lib/pkgconfig is on pkg-config's search path.

Environment:
  PLINTH_USE_SYSTEM_CRYPTO_LIBS=1   Opt out: do nothing, use system libraries.
                                    (Ignored when --prefix is given: that mode
                                    IS the system-wide install.)
...
if [ "${PLINTH_USE_SYSTEM_CRYPTO_LIBS:-0}" = 1 ] && [ -z "$SYSTEM_PREFIX" ]; then
  say "PLINTH_USE_SYSTEM_CRYPTO_LIBS=1: skipping download, expecting system-wide crypto libs."
  exit 0
fi
...
# on the fast already-installed path, so a wiped dist-newstyle or a deleted
# cabal.project.local heals on the next run — without re-downloading.
# ---------------------------------------------------------------------------

write_env_file() {
  mkdir -p "$(dirname "$ENV_FILE")"
  cat > "$ENV_FILE" <<EOF
# Generated by scripts/get-crypto-libs.sh. Source this file, then run cabal.
...
# The program-locations stanza in cabal.project (relative path) covers
# cabal's dependency SOLVER, but the configure step of non-local packages
# (cardano-crypto-class) runs with a different working directory and needs an
# absolute path, which cannot be committed. It is therefore kept in the
# machine-local, gitignored cabal.project.local, inside a marked block that
# this script owns and refreshes.
maintain_cabal_project_local() {
  LOCAL_FILE="$REPO_ROOT/cabal.project.local"
  MARK_BEGIN="-- BEGIN plinth-template crypto-libs (generated by scripts/get-crypto-libs.sh)"
  MARK_END="-- END plinth-template crypto-libs"
  SHIM="$REPO_ROOT/scripts/pkg-config"

  if ! grep -qF "pkg-config-location: $SHIM" "$LOCAL_FILE" 2>/dev/null; then
    tmp="$LOCAL_FILE.tmp.$$"
    ...
    fi
    {
      echo "$MARK_BEGIN"
      echo "package *"
      echo "  pkg-config-location: $SHIM"
      echo "$MARK_END"
    } >> "$tmp"
    mv "$tmp" "$LOCAL_FILE"
    say "Updated $LOCAL_FILE (pkg-config shim location for package configure)."
    say "NOTE: a cabal command that was already running when this file was"
    say "created cannot pick it up; if that build stops with 'Cannot find the"
    say "program pkg-config', simply re-run it (this happens at most once)."
  fi
}
...
[ "$RELEASE_COMMIT" = "$IOHK_NIX_COMMIT" ] \
...
extract_into_prefix() {
  ...
  case "$file" in
    *.pkg)
      ...
      [ -n "$BSDTAR" ] || die "extracting $file requires bsdtar (libarchive)"
      local tmp
      tmp="$(mktemp -d "$DOWNLOADS/expand.XXXXXX")"
      "$BSDTAR" -xf "$file" -C "$tmp" Payload
      (cd "$PREFIX" && "$BSDTAR" -xf "$tmp/Payload")
      rm -rf "$tmp"
      ;;
    *.deb)
...
[ -d "$PREFIX/lib/pkgconfig" ] || die "extraction failed: $PREFIX/lib/pkgconfig missing"
...
case "$PLATFORM" in
  *macos)
    ...
    (
      cd "$PREFIX/lib"
      [ -e libsodium.dylib ]    || ln -s libsodium.*.dylib libsodium.dylib
      [ -e libsecp256k1.dylib ] || ln -s libsecp256k1.*.dylib libsecp256k1.dylib
      [ -e libblst.dylib ]      || true # shipped unversioned already
    )
    ;;
  debian)
    (
...
if [ -n "$SYSTEM_PREFIX" ]; then
  ...
  [ -d "$PREFIX/include" ] && cp -R "$PREFIX/include/." "$SYSTEM_PREFIX/include/"
fi
...
if command -v pkg-config >/dev/null 2>&1; then
  for lib in libsodium libsecp256k1 libblst; do
    v="$(PKG_CONFIG_LIBDIR="$FINAL_PREFIX/lib/pkgconfig" pkg-config --modversion "$lib")" \
    ...
    [ "$p" = "$FINAL_PREFIX" ] || die "$lib resolves to unexpected prefix: $p"
...
if [ -n "$SYSTEM_PREFIX" ]; then
  ...
  say ""
  say "To build against them, make sure pkg-config can see them and tell the"
  say "project to use system libraries:"
  say ""
  say "  export PKG_CONFIG_PATH=\"$SYSTEM_PREFIX/lib/pkgconfig\${PKG_CONFIG_PATH:+:\$PKG_CONFIG_PATH}\""
  say "  export PLINTH_USE_SYSTEM_CRYPTO_LIBS=1"
  say "  cabal build all"
  exit 0
fi
